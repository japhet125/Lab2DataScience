...
ggplot( samp1, aes(x = scientist_work)) +
  ...
  labs(
    x = "", y = "",
    title = "Distribution of sample of size 50 from global_monitor"
  ) +
  coord_flip()
```
**Summarize: The distribution of responses in the sample of size 50 shows that most respondents believe that the work scientists do benefits them, while a smaller portion believe that it does not. This pattern is similar to the population distribution, where 80% believe science benefits them and 20% do not. However, because the sample is randomly selected and much smaller than the population, the proportions in the sample may differ slightly from the population proportions.**
...
**Insert your answer here: **Answer** **I would not expect the sample proportion to exactly match another student's sample proportion because each sample is selected randomly from the population. Different random samples may contain different individuals, which leads to different proportions. However, the proportions should not be drastically different and should generally be close to the true population proportion.**
**
...
**Insert your answer here**
**visualizing: a sample of size 50 and call it samp2**
```{r}
...
ggplot( samp2, aes(x = scientist_work)) +
  ...
  labs(
    x = "", y = "",
    title = "Distribution of sample2 of size 50 from global_monitor"
  ) +
  coord_flip()
```
```{r}
...
```
***Summarizing: with size 50 of samp2 we can see that 39 says it benefits them while 11 says it doesn't benefit them which is different from samp1***

***Using a size of 100 and comparing with a size of 1000***

**sample of 100**
```{r}
samp_100 <- global_monitor %>%
  sample_n(100)
ggplot( samp_100, aes(x = scientist_work)) +
  geom_bar() +
  labs(
    x = "", y = "",
    title = "Distribution of sample2 of size 50 from global_monitor"
...
samp_100 %>%
  count(scientist_work) %>%
  mutate(p_hat_100 = n / sum(n))
```
**Using sample of 1000**
```{r}
samp_1000 <- global_monitor %>%
  sample_n(1000)
ggplot( samp_1000, aes(x = scientist_work)) +
  geom_bar() +
  labs(
    x = "", y = "",
    title = "Distribution of sample2 of size 50 from global_monitor"
...
samp_1000 %>%
  count(scientist_work) %>%
  mutate(p_hat_1000 = n / sum(n))
```
**Answer: The sample proportion from samp2 may differ from the proportion in samp1 because each sample is drawn randomly from the population. Random variation can cause the estimates to differ slightly.**
**If we took additional samples of size 100 and 1000, the sample of size 1000 would provide a more accurate estimate of the population proportion. Larger sample sizes reduce sampling variability and produce estimates that are closer to the true population proportion.***
...
**Insert your answer here: Answer: There are 15,000 elements in sample_props50, corresponding to the 15,000 simulated samples taken from the population. Each element represents the sample proportion of respondents who answered "Doesn't benefit" in a sample of size 50.**
**The sampling distribution of the sample proportions appears approximately bell-shaped and symmetric, resembling a normal distribution. The center of the distribution is around 0.20, which matches the true population proportion of individuals who believe that the work scientists do does not benefit them.****

```{r}
ggplot(sample_props50, aes(x = p_hat)) +
  geom_histogram(aes(y = ..density..), binwidth = 0.02, fill = "lightblue", color = "black") +
...
**Insert your answer here: Answer: we have 25 observation are in this sample_props_small and each observation the number of percentage in each sample of size 10 the number of people that Doesn't benefit from science.****
...
**Insert your answer here: Answer: Each observation in the sampling distribution represents the sample proportion of people who said "Doesn't benefit" in one simulated sample. As the sample size increases from 10 to 50 to 100, the mean of the sampling distribution stays close to the true population proportion of 0.20, while the standard error decreases. This means the distribution becomes less spread out and the estimates become more precise. In addition, the shape becomes more nearly bell-shaped as the sample size increases. If the number of simulations is increased, the overall shape becomes smoother and more stable, but the center and standard error do not change much****
...
**Insert your answer here**
**Answer: using this sample the best point estimate of people who think science enhance their lives is almost 92% of the population**
```{r}
sample_1 <- global_monitor %>%
...
**Insert your answer here**
**Answer: the shape of this sample distribution of the sample proportions appears approximately bell-shaped and symmetric, resembling a normal distribution. The true proportion of those who think the work of science do enchances their lives is aproximatly 0.80 or 80%**
```{r}
global_monitor %>%
  summarise(p = mean(scientist_work == "Benefits"))
```
```{r}
...
**Insert your answer here**
**Answer:The sampling distribution of the sample proportions appears approximately bell-shaped and symmetric, resembling a normal distribution. The center of the distribution is around 0.80, which matches the true population proportion of individuals who believe that the work scientists do does benefits them. we can estimate the proportion to be around 80%**
```{r}
...
10.  Of the sampling distributions from 2 and 3, which has a smaller spread?  If
    ...
    **Answer: The sampling distribution based on the sample size of 150 has the smaller spread. If I want estimates that are more often close to the true population proportion, I would prefer a sampling distribution with a small spread, because that indicates less variability and more precise estimates**
