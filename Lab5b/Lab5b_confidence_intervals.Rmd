...
**Insert your answer here: the percentage of adults in the sample who think climate change affects their local community is 55%**
...
**Insert your answer here: I would not expect the sample proportion to exactly match another student's sample proportion because each sample is selected randomly from the us adults. Different random samples may contain different individuals, which leads to different proportions. However, the proportions should not be drastically different and should generally be close to the true us adults proportion.**
...
**Insert your answer here: “95% confidence” means that if we repeatedly took random samples from the population and constructed a confidence interval from each sample using the same method, about 95% of those intervals would capture the true population proportion.**
...
**Insert your answer here: My confidence interval does capture the true population proportion of 0.62 because 0.62 falls between the lower and upper bounds of the interval. My neighbor’s interval may be different because it is based on a different random sample, but it may also capture the true population proportion.**
...
**Insert your answer here: I would expect about 95% of the confidence intervals to capture the true population proportion, because that is what a 95% confidence level means in repeated sampling.**
...
**Insert your answer here: I would expect about 95% of the 50 confidence intervals, or about 47 or 48 intervals, to contain the true population proportion of 0.62. This proportion will not necessarily be exactly 95% because of random sampling variability. In any one simulation, the observed proportion may be slightly above or below 95%. **
```{r}
samp %>%
  specify(response = climate_change_affects, success = "Yes") %>%
  generate(reps = 1000, type = "bootstrap") %>%
...
**Insert your answer here: It will depends on the value I chose if the value is less than .95 it will be narrow confidence level, if it is greater than 0.95 it will be wider. This is because greater confidence requires a wider interval to capture the true population proportion more often.**
```{r}
samp %>%
  specify(response = climate_change_affects, success = "Yes") %>%
  generate(reps = 1000, type = "bootstrap") %>%
...
**Insert your answer here: Using a 75% confidence level, I am 75% confident that the true proportion of U.S. adults who think climate change affects their local community is between 0.467 and 0.617 but those values may change since we are using random. **
```{r}
samp %>%
  specify(response = climate_change_affects, success = "Yes") %>%
  generate(reps = 1000, type = "bootstrap") %>%
...
**Insert your answer here: If I construct 50 confidence intervals at the 75% confidence level, I would expect about 75% of them, or around 37 to 38 intervals, to capture the true population proportion. The exact percentage may not be exactly 75% in a particular run because of random sampling variability, but it should usually be close. **
...
**Insert your answer here: I expect a 90% confidence interval to be wider than a 75% confidence interval, because higher confidence requires a wider interval. Using the infer package, I calculated a 90% confidence interval of [lower, upper]. I am 90% confident that the true proportion of U.S. adults who think climate change affects their local community lies between these bounds. Using the app, I would expect about 90% of the intervals, or about 45 out of 50, to capture the true population proportion. **
...
**Insert your answer here: Using the app, and experimenting with different sample sizes the widths of the intervals change as sample size changes. As the sample size increases, the width of the confidence interval decreases. As the sample size decreases, the width of the confidence interval increases. Larger samples provide more precise estimates, which leads to narrower confidence intervals**
...
**Insert your answer here: For a fixed sample size, increasing the number of bootstrap samples does not substantially change the width of the confidence interval. It mainly makes the bootstrap estimate more stable and reliable. In general, changing the number of bootstrap samples does not change the standard error in a meaningful way; the standard error is driven primarily by the sample size and the variability in the data. **
